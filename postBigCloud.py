...
def postCloudBig(data):
    url = '{}/south/file/uploadCloud/slice'.format(host)
...
def postCloudPartFile(f,sliceNo,uploadKey,token):
    j = 10
    id = []
    ...
    print(response.status_code)
    print(response.content)
    if response.status_code==200 and json.loads(response.content.decode())["code"] == "1":
        print('upload filepart success')
    else:
        print('upload filepart failed')
def postCloudMerge(uploadKey,taskId):
    print(uploadKey)
    ...
    print(response.status_code)
    print(response.content)
    postCloudUploadStatus(taskId,100,token)
def postCloudUploadStatus(taskId,status,token):
...
if __name__ == "__main__":
    ...
    for uf in  params["file_list"]:
        data = createUp(uf["local_path"],params["taskId"],token)
        td = postCloudBig(data)
        print(td)
        td = json.loads(td.decode())
        print(uf)
        f = open(uf["local_path"],"rb")
        nums = td["data"]["sliceCount"]
        partSize = td["data"]["size"]
        uploadKey = td["data"]["uploadKey"]
        for i in range(nums):
            postCloudPartFile(f.read(partSize),str(i),uploadKey,token)
            postCloudUploadStatus(params["taskId"],int(i*100.0/nums),token)
        f.close()
        postCloudMerge(uploadKey,params["taskId"]) 
