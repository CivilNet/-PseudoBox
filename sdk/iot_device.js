...
class IotDevice extends EventEmitter {
    constructor({serverAddress = "<ip>:<port>", productName, deviceName, secret, clientID, storePath} = {}) {
    ...
    handleCommand({commandName, requestID, encoding, payload, expiresAt, commandType = "cmd"}) {
        console.log(commandName,requestID,encoding,payload,expiresAt)
        if (expiresAt == null || expiresAt > Math.floor(Date.now() / 1000)) {
            ...
            if (commandName.startsWith("$")) {
                ...
                this.emit("command", commandName, requestID, data, respondCommand)
            }
        }
    }
